...
function visualize(tp::Tp; camera = (0, 30), opacity=0.5)
    tp_offset=0.5
    offset=3.0
    ...
    wireframe!(p, xs, ys, (x, y) -> tp.zfunc(x, y), camera = camera, opacity=0.9)
    surface!(p, tp_xs, tp_ys, (x, y) -> tangent_plane(tp, x, y), camera = camera, opacity=0.8)
